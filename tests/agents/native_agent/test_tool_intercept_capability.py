...
# ============================================================================
# 5.4: wrap_tool_execute() converts exceptions to first-class failures
# ============================================================================
...
async def test_wrap_tool_execute_catches_exception(
    ...
    """Ordinary exceptions become pydantic-ai first-class tool failures."""
    cap = make_capability(mock_hook_manager)
    ctx = make_run_context()
...
@pytest.mark.anyio
async def test_wrap_tool_execute_propagates_declared_tool_result_failure(
    mock_hook_manager: MagicMock,
) -> None:
    """``ToolResult.is_error`` becomes the same first-class failure signal."""
