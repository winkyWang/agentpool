...
class SessionPoolMessagingMixin:
    ...
    _subagent_inactivity_timeout_seconds: float | None

    async def _wait_for_subagent_event(self, bus_queue: asyncio.Queue[Any]) -> Any:
        """Wait for the next delegated event using the configured silence deadline."""
        timeout_seconds = self._subagent_inactivity_timeout_seconds
        if timeout_seconds is None:
            return await bus_queue.get()
        return await asyncio.wait_for(bus_queue.get(), timeout=timeout_seconds)
