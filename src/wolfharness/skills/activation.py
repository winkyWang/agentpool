...
SkillActivationSource = Literal["metadata", "matcher", "always_active", "all", "explicit"]
...
async def record_skill_trace(
    ...
    *,
    ...
    """Record Skill exposure or activation and publish it when an EventBus exists."""
    ...
    if run_ctx.event_bus is not None:
        from wolfharness.agents.events import CustomEvent
...
async def activate_skills(
    ...
    *,
    ...
    """Activate distinct Skill bodies without truncating or partially mutating."""
    requested = set(requested_skills)
    active = (
        agent_ctx.run_ctx.activated_skills
